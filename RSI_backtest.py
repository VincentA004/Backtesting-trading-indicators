...
logging.basicConfig(filename = f'RSI_complete_backtest.log', filemode= 'w', level = logging.INFO)
    ...
    for ticker in ticker_list:
        try:
            ...
            for x in range(len(dates)):
                if x > 0 and x+1 < len(dates):
                    if clean_data.loc[dates[x], "RSI"] > n1 and clean_data.loc[dates[x-1],"RSI"] < n1:
                        if(len(buy) < 1):
                            buy.append(dates[x+1])
                            buy_price.append(clean_data.loc[dates[x+1], "Open"])
                    elif clean_data.loc[dates[x], "RSI"] < n2 and clean_data.loc[dates[x-1],"RSI"] > n2:
                        if len(buy) > 0:
                            total_count += 1
                            avg_invest += 100 * clean_data.loc[buy[0], 'Open']
                            total_profit += 100*(clean_data.loc[dates[x+1],'Open'] - clean_data.loc[buy[0], 'Open'])
                            sell_price.append(clean_data.loc[dates[x+1], "Open"])
                            if clean_data.loc[buy[0], 'Open'] < clean_data.loc[dates[x+1],'Open']:
                                right_count += 1
                            del buy[0]
        except:
