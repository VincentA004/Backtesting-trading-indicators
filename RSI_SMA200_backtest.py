...
logging.basicConfig(filename = f'RSI_SMA200_real_backtest3.log', filemode= 'w', level = logging.INFO)
    ...
    try:
        ...
        for x in range(len(dates)):
            if x > 0 and x < len(dates)-1:
                if (clean_data.loc[dates[x], "SMA50"] > clean_data.loc[dates[x],"Close"]) and clean_data.loc[dates[x], "RSI"] > 45 and clean_data.loc[dates[x-1],"RSI"] < 45:
                    if len(buy) < 1:
                        buy.append(dates[x])
                        buy_price.append(clean_data.loc[dates[x+1], "Close"])
                    ...
                    if len(buy) > 0:
                        ...
                        sell_price.append(clean_data.loc[dates[x+1], "Close"])
                        if clean_data.loc[buy[0],'Close'] < clean_data.loc[dates[x],'Close']:
                            right_count += 1
                        del buy[0]
                '''
                elif len(buy) > 0:
                    if clean_data.loc[dates[x], 'Close'] < (clean_data.loc[buy[0],'Close'] * .8):
                        total_count += 1
                        avg_invest += 100 * clean_data.loc[buy[0], 'Close']
                        total_profit += 100*(clean_data.loc[dates[x],'Close'] - clean_data.loc[buy[0], 'Close'])
                        sell_price.append(clean_data.loc[dates[x+1], "Close"])
