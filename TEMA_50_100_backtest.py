...
for ticker in ticker_list:
    try:
        ...
        for x in range(len(dates)):
            if x > 0:
                if clean_data.loc[dates[x], "TEMA50"] > clean_data.loc[dates[x], "TEMA100"] and clean_data.loc[dates[x-1], "TEMA50"] < clean_data.loc[dates[x-1], "TEMA100"]:
                    if len(buy) < 1:
                        buy.append(dates[x])
                elif clean_data.loc[dates[x], "TEMA50"] < clean_data.loc[dates[x], "TEMA100"] and clean_data.loc[dates[x-1], "TEMA50"] > clean_data.loc[dates[x-1], "TEMA100"]:
                    if len(buy) > 0:
                        total_count += 1
