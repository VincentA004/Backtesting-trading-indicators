...
for ticker in ticker_list:
    try:
        ...
        for x in range(len(dates)):
            if (x+5) < len(dates):
                if clean_data.loc[dates[x], 'STD']/clean_data.loc[dates[x], 'Close'] <= .008:
                    total_count += 1
                    ...
                    if (clean_data.loc[dates[x+5], 'Close'] < 1.02*clean_data.loc[dates[x], 'Close'] ) and (clean_data.loc[dates[x+5], 'Close'] > .98*clean_data.loc[dates[x], 'Close'] ):
                        right_count += 1
