...
logging.basicConfig(filename = f'SMA200_real_backtest.log', filemode= 'w', level = logging.INFO)
        ...
        try:
            ...
            for x in range(len(dates)):
                if x > 0 and x+1 < len(dates):
                    ...
                    elif clean_data.loc[dates[x], "SMA200"] < clean_data.loc[dates[x],"Close"] and clean_data.loc[dates[x-1],"SMA200"] > clean_data.loc[dates[x-1],"Close"]:
                        if len(buy) > 0:
                            total_count += 1
