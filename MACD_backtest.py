...
for ticker in ticker_list:
    try:
        ...
        for x in range(len(dates)):
            if x > 0:
                if clean_data.loc[dates[x], "MACDHisto"] > 0 and clean_data.loc[dates[x-1],"MACDHisto"] < 0:
                    buy.append(dates[x])
                elif clean_data.loc[dates[x], "MACDHisto"] < 0 and clean_data.loc[dates[x-1],"MACDHisto"] > 0:
                    if len(buy) > 0:
                        total_count += 1
